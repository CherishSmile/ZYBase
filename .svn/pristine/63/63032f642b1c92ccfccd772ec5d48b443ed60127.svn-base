...
#import "ZYChineseSort.h"

//特殊字符统一合并后的  显示字符
#define AbnormalLetter @"#"

@interface ZYChineseSort()
//用将需要排序的对象封装在ZYChineseSort对象中，包含排序的字符串，对象，首字母三个属性
//进行比较的字符串，
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((ZYChineseSort*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((ZYChineseSort*)object).pinYin substringToIndex:1];
        NSString *string = ((ZYChineseSort*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        ZYChineseSort *chineseString = [[ZYChineseSort alloc]init];
        ...
        chineseString.string = [ZYChineseSort RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (ZYChineseSort* object in tempArray) {
    ...
    if (BMLog==1) {
        ...
        NSLog(@"ZYChineseSort user time in %f ms", useTime *1000.0);
    }
