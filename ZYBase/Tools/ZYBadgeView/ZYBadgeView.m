...
#error ZYBadgeView must be compiled with ARC.
#endif
...
// Soon ZYBadgeView will require iOS 7 and we'll be able to use the new methods.
#if  __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_7_0
    #define ZYBadgeViewSilenceDeprecatedMethodStart()   _Pragma("clang diagnostic push") \
                                                        _Pragma("clang diagnostic ignored \"-Wdeprecated-declarations\"")
    #define ZYBadgeViewSilenceDeprecatedMethodEnd()     _Pragma("clang diagnostic pop")
#else
    #define ZYBadgeViewSilenceDeprecatedMethodStart()
    #define ZYBadgeViewSilenceDeprecatedMethodEnd()
#endif

static const CGFloat ZYBadgeViewShadowRadius = 1.0f;
static const CGFloat ZYBadgeViewHeight = 16.0f;
static const CGFloat ZYBadgeViewTextSideMargin = 8.0f;
static const CGFloat ZYBadgeViewCornerRadius = 10.0f;

// Thanks to Peter Steinberger: https://gist.github.com/steipete/6526860
static BOOL ZYBadgeViewIsUIKitFlatMode(void)
{
...
{
    ZYBadgeView *badgeViewAppearanceProxy = ZYBadgeView.appearance;

    badgeViewAppearanceProxy.backgroundColor = UIColor.clearColor;
    badgeViewAppearanceProxy.badgeAlignment = ZYBadgeViewAlignmentTopRight;
...
+ (void)initialize
{
    if (self == ZYBadgeView.class)
    {
        [self applyCommonStyle];

        if (ZYBadgeViewIsUIKitFlatMode())
        {
            [self applyIOS7Style];
        }
        else
...
- (void)layoutSubviews
{
    ...
    const CGFloat viewWidth = MAX(_badgeMinWidth, textWidth + ZYBadgeViewTextSideMargin + (marginToDrawInside * 2));
    const CGFloat viewHeight = ZYBadgeViewHeight + (marginToDrawInside * 2);
    ...
    switch (self.badgeAlignment) {
        case ZYBadgeViewAlignmentTopLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case ZYBadgeViewAlignmentTopRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case ZYBadgeViewAlignmentTopCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case ZYBadgeViewAlignmentCenterLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case ZYBadgeViewAlignmentCenterRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case ZYBadgeViewAlignmentBottomLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case ZYBadgeViewAlignmentBottomRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case ZYBadgeViewAlignmentBottomCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case ZYBadgeViewAlignmentCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        default:
            NSAssert(NO, @"Unimplemented ZYBadgeAligment type %lul", (unsigned long)self.badgeAlignment);
    }
...
{
    ZYBadgeViewSilenceDeprecatedMethodStart();
    return [self.badgeText sizeWithFont:self.badgeTextFont];
    ZYBadgeViewSilenceDeprecatedMethodEnd();
}

#pragma mark - Setters

- (void)setBadgeAlignment:(ZYBadgeViewAlignment)badgeAlignment
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (anyTextToDraw)
    {
        ...
        UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:rectToDraw byRoundingCorners:(UIRectCorner)UIRectCornerAllCorners cornerRadii:CGSizeMake(ZYBadgeViewCornerRadius, ZYBadgeViewCornerRadius)];
        ...
        CGContextSaveGState(ctx);
        {
            ...
            CGContextSetShadowWithColor(ctx, self.badgeShadowSize, ZYBadgeViewShadowRadius, self.badgeShadowColor.CGColor);
